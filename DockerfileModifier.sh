...
RUN --mount=type=cache,target=/root/.npm \\
    echo "Checking if package exists: ${FIRECRAWL_MCP_PKG}" && \\
...
LABEL org.opencontainers.image.description="Firecrawl MCP (mcp-proxy stdio<->HTTP bridge)"

# mcp-proxy and HAProxy concurrency defaults (overridable at runtime).
ENV MCP_PROXY_STATELESS=false
ENV FIRECRAWL_MAX_MEM_MB=4096
